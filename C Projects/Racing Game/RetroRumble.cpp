...
#define TOP 3
#define BOTTOM 28
// Feel free to change these values to get the look you want!
#define YELLOW 14
#define CYAN 3
#define RED 12
#define GREEN 10
#define LIGHT_CYAN 11
#define DARK_CYAN 9      // Was 211, now standard Light Blue
#define DARK_GREEN 2
#define BLUE 1
#define GREY 8
#define GOLDEN 6
...
void drawToBuffer(int x, int y, char c, int color) {
    if (x >= 0 && x < SCREEN_WIDTH && y >= 0 && y < SCREEN_HEIGHT) {
        consoleBuffer[y * SCREEN_WIDTH + x].Char.AsciiChar = c;
        consoleBuffer[y * SCREEN_WIDTH + x].Attributes = color;
    }
}

void drawStringToBuffer(int x, int y, const char* str, int color) {
    for (int i = 0; str[i] != '\0'; ++i) {
        drawToBuffer(x + i, y, str[i], color);
    }
}
...
void canvas(COORD upper_left, COORD lower_right, char ch1, char ch2, char ch3, char ch4, int color)
{
    ...
    for (i = upper_left.X; i <= lower_right.X; i++) {
        drawToBuffer(i, upper_left.Y, ch1, color);
        drawToBuffer(i, lower_right.Y, ch3, color);
    }
...
// RENAMED from draw_vehicle. This is for the buffered game loop.
void draw_vehicle(char str[10][10], COORD C, int flag, int color)
{
    int i, j = 0;
    for (i = C.Y; i < C.Y + 5; i++) {
        for (int k = 0; k < 4; k++) { // All vehicles are 4 characters wide
            if (flag) {
                drawToBuffer(C.X + k, i, str[j][k], color);
            }
        }
        j++;
    }
...
// NEW function. This is the original direct-drawing version for menus.
void draw_vehicle_direct(char str[10][10], COORD C, int flag, int color)
{
    setColor(color);
    int i, j = 0;
    if (flag) {
        for (i = C.Y; i < C.Y + 5; i++) {
            gotoxy(C.X, i);
            // Print only the first 4 characters
            for(int k=0; k<4; k++) {
                printf("%c", str[j][k]);
            }
            j++;
...
void go_track(char str[100][100], COORD C, int color)
{
    int i, j = C.Y;
    for (i = index; i < 30; i++) {
        drawStringToBuffer(C.X, j, str[i], color);
        j++;
    }
    for (i = 0; i < index; i++) {
        drawStringToBuffer(C.X, j, str[i], color);
...
void multiplay()
{
    // No system("cls") is needed. The buffer handles screen clearing.
    char command;
    int flag = 0, j;
    long timecount = 0;

    int game_speed_delay = 80; // Game speed is controlled by this delay. Lower is faster.
    DWORD last_game_tick = 0;

    // Reset player positions and game state for a new game
    player1.X = 41; player1.Y = 23;
    player2.X = 77; player2.Y = 23;
    i = 0;
    index = 29;
    ...
    while (1)
    {
        ...
        // --- 1. Input Handling (no changes needed) ---
        if (kbhit())
        {
            flag = 1;
            command = getch();
        ...
        // With double buffering, we don't need to manually erase the old car.
        // We just update its coordinates.
        if (flag)
        {
            if (command == 'a')      player1.X -= 4;
            else if (command == 's') player1.X += 4;
            else if (command == 'j') player2.X -= 4;
            else if (command == 'k') player2.X += 4;
            flag = 0;
        }

        // --- 3. Update Game Logic (Obstacles, Road) ---
        if (frame_start - last_game_tick > game_speed_delay) {
            index--; // Scroll the road
            if (index < 0) index = 29; // Loop the road animation

            if (index % 10 == 0) threat_generator(); // Spawn new threats

            // Update positions of all existing threats
            for (j = 0; j < i; j++) {
                if (t[j].pos.X) {
                    COORD C = linear(t[j].pos);
                    t[j].pos = C;
                }
            }
            last_game_tick = frame_start;
        }

        // --- 4. Drawing Phase (The Core of Double Buffering) ---
        clearBuffer(); // Start with a fresh, blank slate in memory
        ...
        // Draw the dynamic elements (players and threats)
        draw_vehicle(car[p1], player1, 1, car_color[p1]);
        draw_vehicle(car[p2], player2, 1, car_color[p2]);
        for (j = 0; j < i; j++) {
            if(t[j].pos.X) draw_vehicle(obstacles[t[j].vehicle_type], t[j].pos, 1, color[t[j].vehicle_type]);
        }

        render(); // Display the perfectly drawn frame to the screen all at once

        // --- 5. Game Over Check & Timing Control ---
        int g = game_over();
        if (g)
        {
            multi_game_crash(g);
        ...
        {
            timecount = 0;
            if (game_speed_delay > 50) game_speed_delay--; // Increase speed over time
        }

        DWORD frame_duration = GetTickCount() - frame_start;
...
void scoreboard()
{
    ...
    char coin_str[20];
    sprintf(coin_str, "$%d", coins);

    drawStringToBuffer(90, 12, "COINS:", GREEN);
    drawStringToBuffer(91, 15, "      ", YELLOW); // Clear previous score
    drawStringToBuffer(91, 15, coin_str, YELLOW);
}
...
{
    // --- STATE RESET ---
    // This is the crucial fix. We reset the game state every time a new game starts.
    i = 0;
    index = 29;
    // --- END FIX ---
    ...
    while (1)
    {
        ...
        if (flag) {
            if (command == 75) player1.X -= 4;
            else if (command == 77) player1.X += 4;
            else if (command == 27) {
                pause_write(e, player1, coins);
                P[avatar].coins += coins; // Save the coins collected before quitting
                writeData();
                coins = 0;
                return;
            }
        ...
        if (frame_start - last_game_tick > e) {
            index--;
            if(index < 0) index = 29;
        ...
        int g = game_over();
        if (g) {
            mono_game_crash(); // This now handles messages and score saving
            P[avatar].coins += coins; // Add session coins to total
            writeData();
            player1.X = 41; player1.Y = 23; // Reset player position for next pause_read
            pause_write(80, player1, 0); // Reset saved state to a fresh game
            coins = 0; // Reset session coins
...
// REVERTED to original direct-drawing method for menus.
void setup()
{
    ...
    gotoxy(0,0); // Good practice to reset cursor
}
...
int purchase()
{
    ...
    for(i = 0; i < 7; i++)
    {
        draw_vehicle_direct(car[i], pos[i], 1, car_color[i]);
...
int select_car()
{
    ...
    while(1)
    {
        ...
        if(ch == 77 && pos < 6)
        {
            ...
            {
                draw_vehicle_direct(car[0], C, 0, car_color[0]);
                draw_vehicle_direct(car[pos], C, 1, car_color[pos]);
                selection = pos;
            }
        }
